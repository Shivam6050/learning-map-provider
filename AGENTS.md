...
# AI Pipeline & Gemini Integration Rules

- **Provider**: Use Google Gemini API, configured with `GEMINI_API_KEY`.
- **Model Standard**: Use `gemini-2.5-flash` (or fallback candidate `gemini-1.5-flash`) with native JSON mode (`response_mime_type: "application/json"`).
