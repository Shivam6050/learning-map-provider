...
export function CurrencySwitcher() {
  const { currency, setCurrency } = useCurrency();
  return <select name="displayCurrency" aria-label="Display currency" value={currency ?? "USD"} onChange={e => setCurrency(e.target.value as Currency)} className="currency-select rounded-lg border border-slate-700 px-2 py-1 text-xs">
